...
class nntpreader_db {
			...
			foreach ($groups as $group) {
				if (! $lookGroups || ($lookGroups && in_array($group, $doGroups))) {
					...
					if (! in_array($group, $existGroups)) {
						...
						$fields_values = array ('tstamp' => time(), 'crdate' => time(), 'name' => $group, 'nntp_name' => $group, 'server' => $this->parent->server['uid'], 'messages' => $messages, 'pull' => 0, 'pid' => $this->pid);
						$res = $GLOBALS['TYPO3_DB']->exec_INSERTquery('tx_nntpreader_groups', $fields_values, $no_quote_fields = FALSE);
						$action['new'] ++;
					} else {
						$res = $GLOBALS['TYPO3_DB']->exec_UPDATEquery('tx_nntpreader_groups', 'nntp_name="' . $group . '"', array ('tstamp' => time(), 'messages' => $messages), $no_quote_fields = FALSE);
						$action['updated'] ++;
					}
				}
			}
	...
	public function updateMessage($m, $msg) {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_UPDATEquery('tx_nntpreader_text', 'uid=' . $textuid, array ('body' => $msg->body['text'], 'header' => $msg->headerString, 'subject' => $this->imap_utf8($this->cs->conv($msg->header->subject, 'iso-8859-1', 'utf-8'))), $no_quote_fields = FALSE);
	}
	...
	public function saveMessage($msg, $group, $pid) {
		...
		if (is_array($msg->structure->parameters) && strtoupper($msg->structure->parameters[0]->attribute) == 'CHARSET') {
			...
			if ($charset == 'us-ascii') {
				...
				if ($charset != 'utf-8')
					$msg->body['text'] = $this->cs->conv($msg->body['text'], $charset, 'utf-8'); # imap_utf8($msg->body['text']);
			}
		} else {
			$msg->body['text'] = $this->imap_utf8($msg->body['text']);
		}
		
		$resInsert = $GLOBALS['TYPO3_DB']->exec_INSERTquery('tx_nntpreader_text', array ('pid' => $pid, 'crdate' => time(), 'tstamp' => strtotime($msg->header->date), 'body' => $msg->body['text'], 'header' => $msg->headerString, 'subject' => $this->imap_utf8($this->cs->conv($msg->header->subject, 'iso-8859-1', 'utf-8')), 'newsgroup' => $group['uid']));
...
'imap_utf8' => imap_utf8($this->cs->conv($msg->header->sender[0]->personal, $charset, 'utf-8')),
		...
		$insertData = array ('pid' => $pid, 'crdate' => time(), 'tstamp' => time(), 'msgid' => $msg->header->message_id, 'msguid' => $msg->uid, 'msgnumber' => $msg->number, 'newsgroup' => $group['uid'], 'emailfrom' => $msg->header->sender[0]->mailbox . '@' . $msg->header->sender[0]->host, 'namefrom' => $this->imap_utf8($this->cs->conv($msg->header->sender[0]->personal, 'iso-8859-1', 'utf-8')), 'subject' => $this->imap_utf8($this->cs->conv($msg->header->subject, 'iso-8859-1', 'utf-8')), 'maildate' => strtotime($msg->header->date), 'mailsize' => strlen($msg->header->headerString) + strlen($msg->body['text']), 'msgreference' => $reply, #$msg->header->in_reply_to, #$msg->header->references,
'type' => $msg->structure->type, 'textuid' => $id, 'files' => count($msg->files) > 0 ? implode(',', $msg->files) : '');
